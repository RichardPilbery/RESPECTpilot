...
writeCSV <- function(df,nameDF) {
  ...
  #outputDF[,8] <- df$orig_participant_id
  #outputDF[,9] <- df$orig_ecg_id
  # colnames(outputDF) <- c("row-id","con1","con0","participant-id","ecg-id","diag-message","diag-no-message","orig-part-id","orig-ecg-id")
  colnames(outputDF) <- c("row-id","con1","con0","participant-id","ecg-id","diag-message","diag-no-message")
  print(head(outputDF))
  filename <- paste("~/Dropbox/RData/WinBUGS-correct/",nameDF,"-XXXXXXXX.csv",sep="")
  write.csv(outputDF, file=filename,row.names=F)
}

#writeCSV(newCC,"FSD99CC")
...
writeList <- function(df,nameDF) {
  ...
  initsname <- ""
  if(nameDF=="FSD99CC") {
    filename <- paste("/Users/tricky999/Dropbox/RData/WinBUGS-correct/",nameDF,".txt",sep="")
    initsname <- c('WinBUGS-correct/inits.txt')
  }
  else {
    filename <- paste("/Users/tricky999/Dropbox/RData/WinBUGS-incorrect/",nameDF,".txt",sep="")
...
#bugsTestC <- bugs(data=c("correct-data.txt"), inits=c("inits.txt","inits.txt","inits.txt"), parameters.to.save=c("beta","ecg","sds","sde"), model.file = "model.txt",program=c("WinBUGS"),n.chains = 3,n.iter = 1000000,n.burnin=6000,n.thin=5,digits=5, useWINE=TRUE,bugs.directory = "/Users/tricky/Wine Files/drive_c/Program Files/WinBUGS14", working.directory = "./winBUGS-correct", WINE="/Applications/Wine.app/Contents/Resources/bin/wine",WINEPATH="/Applications/Wine.app/Contents/Resources/bin/winepath",debug=F,DIC=TRUE,codaPkg=TRUE)

````
Instead, having run the data and generated the results, it's possible to load them in directly.  The generateOR function will load in the correct coda file and calculate the OR, CI and ICC values for ECG and Participants.
````{r process-WinBUGS-data, results='hide',message=FALSE,warning=FALSE}
...
chainoneI <- generateOR(1,'~/Dropbox/RData/winBUGS-incorrect/')
chaintwoI <- generateOR(2,'~/Dropbox/RData/winBUGS-incorrect/')
chainthreeI <- generateOR(3,'~/Dropbox/RData/winBUGS-incorrect/')

chainoneC <- generateOR(1,'~/Dropbox/RData/winBUGS-correct/')
chaintwoC <- generateOR(2,'~/Dropbox/RData/winBUGS-correct/')
chainthreeC <- generateOR(3,'~/Dropbox/RData/winBUGS-correct/')
...
|  **ICC ECG**        | `r chainoneI[3]`   | `r chaintwoI[3]`     |  `r chainthreeI[3]` |
| **ICC Participants** | `r chainoneI[4]` | `r chaintwoI[4]`      |  `r chainthreeI[4]` |
...
|  **ICC ECG**        | `r chainoneC[3]`   | `r chaintwoC[3]`     |  `r chainthreeC[3]` |
| **ICC Participants** | `r chainoneC[4]` | `r chaintwoC[4]`      |  `r chainthreeC[4]` |
