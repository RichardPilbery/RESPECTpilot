...
```
The code chunk above tells us that **`r numberConsented`** participants consented, and of those, **`r numberStage1Finish`** completed stage 1, i.e. viewed all of the first 12 ECGs, and **`r numberStage2Finish`** completed the second stage and therefore, provide a full data set.
...
```
From the code chunk above, **`r numberParticipantsFailFinishStage2`** participants who completed stage 1, failed to complete stage 2.
In addition:
...
FinalStudyData <- ddply(FinalStudyData,c("participant_id","ecg_id"),transform)
FinalStudyData99 <- ddply(FinalStudyData99,c("participant_id","ecg_id"),transform)
```
